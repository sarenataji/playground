...
export default async function handler(req, res) {
  ...
  if (!url || !key || !salt) return res.status(503).json({ error: 'The mailbox isn’t open yet. Please come back soon.' });
  // Vercel overwrites this header. Never trust a client-provided forwarded-for chain.
  ...
  try {
    const upstream = await fetch(`${url.replace(/\/$/, '')}/rest/v1/rpc/submit_guest_note`, {
      method: 'POST',
    ...
    if (!upstream.ok) {
      ...
      return res.status(503).json({ error: 'Your note couldn’t be saved. It’s still here—please try again.' });
    }
